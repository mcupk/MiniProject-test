...
def get_input():
    ...
    v_AcademicSemester = st.sidebar.radio('AcademicSemester',['1'])
    ...
    v_Q2 = st.sidebar.checkbox('Quality of life', ['1','0'])
    v_Q3 = st.sidebar.checkbox('Campus and facilities', ['0','1'])

    st.sidebar.subheader('Source of information for this application:')
    v_Q4 = st.sidebar.checkbox('modern and ready-to-use learning support and facilities', ['0','1'])
    v_Q8 = st.sidebar.checkbox('Facebook Admission@MFU', ['0','1'])
    v_Q9 = st.sidebar.checkbox('Facebook MFU', ['0','1'])
    v_Q13 = st.sidebar.checkbox('school teachers', ['0','1'])
    v_Q14 = st.sidebar.checkbox('education exhibitions', ['0','1'])
    v_Q18 = st.sidebar.checkbox('other sources', ['0','1'])
    v_Q19 = st.sidebar.checkbox('https://admission.mfu.ac.th', ['0','1'])
    v_Q20 = st.sidebar.checkbox('https://www.mfu.ac.th', ['0','1'])
    v_Q21 = st.sidebar.checkbox('other educational websites', ['0','1'])

    st.sidebar.subheader('Factor to apply for MFU :')
    v_Q23 = st.sidebar.checkbox('easy/convenient transportation', ['0','1'])
    v_Q24 = st.sidebar.checkbox('suitable cost', ['0','1'])
    v_Q26 = st.sidebar.checkbox('learning in English', ['0','1'])
    v_Q28 = st.sidebar.checkbox('excellence in learning support and facilities', ['0','1'])
    v_Q29 = st.sidebar.checkbox('provision of preferred major', ['0','1'])
    v_Q30 = st.sidebar.checkbox('environment and setting motivate learning', ['0','1'])
    v_Q31 = st.sidebar.checkbox('experienced and high-quality instructors', ['0','1'])

    st.sidebar.subheader('IF your application fails, will you try again?:')
    v_Q34 = st.sidebar.checkbox('try the same major', ['0','1'])
    v_Q35 = st.sidebar.checkbox('try a different major', ['0','1'])

    st.sidebar.subheader('Reason for apply for the major:')
    v_Q37 = st.sidebar.checkbox('suggestion by school teacher', ['0','1'])
    v_Q38 = st.sidebar.checkbox('suggestion by family', ['0','1'])
    v_Q40 = st.sidebar.checkbox('chance of getting a job after graduation', ['0','1'])
    v_Q41 = st.sidebar.checkbox('less competitive than other universities', ['0','1'])
